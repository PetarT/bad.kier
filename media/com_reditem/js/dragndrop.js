(function($) {
	$.fn.dragndrop = function(options) {
		...
		function createStatusbar(obj) {
			...
			this.abort = $("<div class='abort'>" + Joomla.JText._("COM_REDITEM_UPLOAD_ABORT") + "</div>").appendTo(this.statusbar);
		...
		function responseHandlerProcess(data, status, parentDiv) {
			...
			if (uploadType == 'gallery') {
				...
				status.deleteFile = $("<div class='deleteFile'><a class='dragndrop-delete-file'><img src='' alt='" + Joomla.JText._("COM_REDITEM_UPLOAD_DELETE_FILE") + "'/></a></div>").appendTo(status.statusbar);
				...
				preview.attr('src', img_preview_path + data.trim()).load(function() {
					if (isCropEnable) {
						...
						// Add new crop area
						divMedia.append('<div class="div-cropping-outer"><a id="btnCrop_gallery_' + uindex + '" class="btn btnCrop">' + Joomla.JText._("COM_REDITEM_FEATURE_CROP_BTN_LBL") + '</a></div>');
			...
			if (uploadType == 'image') {
				...
				$('#img_preview_' + uploadTarget).load(function() {
					if (isCropEnable) {
						...
						// Add new crop area
						imgPreview.append('<div class="div-cropping-outer"><a id="btnCrop_' + uploadTarget + '" class="btn btnCrop">' + Joomla.JText._("COM_REDITEM_FEATURE_CROP_BTN_LBL") + '</a></div>');
		...
		function uploadValidation(file) {
			if (file.type == "") {
				alert(Joomla.JText._("COM_REDITEM_UPLOAD_FILE_INVALID"));
				return false;
			} else {
				var extTemp = file.name.split('.');

				if ($.inArray(extTemp[extTemp.length - 1], allowExt) < 0) {
					alert(Joomla.JText._("COM_REDITEM_UPLOAD_FILE_INVALID"));
					return false;
				}
			...
			if (allowSize > 0) {
				if (file.size > allowSize) {
					alert(Joomla.JText._("COM_REDITEM_UPLOAD_FILE_TOO_BIG"));
					return false;
				}
			}
		...
		if (isEventSupported('dragenter') && isEventSupported('dragover') && isEventSupported('drop')) {
			...
			if (uploadType == 'image') {
				dragText = Joomla.JText._("COM_REDITEM_ITEM_DRAG_AN_IMAGE");
			}

			if (uploadType == 'gallery') {
				dragText = Joomla.JText._("COM_REDITEM_ITEM_DRAG_IMAGES");
			}

			if (!isFileAPIEnabled()) {
				dragText = Joomla.JText._("COM_REDITEM_ITEM_DRAG_FEATURE_NOT_SUPPORT");
			}
			...
			mainObject.after('<input type="hidden" id="cform_dragndrop_upload' + uid + '" name="cform[dragndrop][' + uploadTarget + '][]" value="" />');

			// Include Browse option enabled
			if ((includeBrowse == 1) && isFileAPIEnabled()) {
				$('<span>').attr('href', 'javascript:void(0)')
					.html(Joomla.JText._("COM_REDITEM_ITEM_DRAG_AND_DROP_BROWSE"))
			...
			obj.on('drop', function(e) {
				...
				if ((e.type === "drop") && isFileAPIEnabled()) {
					...
					if (((uploadType == 'file') || (uploadType == 'image')) && (files.length > 1)) {
						alert(Joomla.JText._("COM_REDITEM_UPLOAD_1_FILE_ONLY"));
						return;
					}
