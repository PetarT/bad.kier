					...
					foreach ($jsonSearch as $column => $value)
					{
						if (is_array($value))
						{
							...
							foreach ($value as $val)
							{
								...
								$tmpWhere[] = $db->qn('cf.' . $column) . ' LIKE ' . $db->quote($db->escape($val, true));
								$tmpWhere[] = $db->qn('cf.' . $column) . ' LIKE ' . $db->quote('%' . json_encode($db->escape($val, true)) . '%');
						...
						else
						{
							...
							$where[] = $db->qn('cf.' . $column) . ' LIKE ' . $db->quote('%' . json_encode($db->escape($value, true)) . '%');
