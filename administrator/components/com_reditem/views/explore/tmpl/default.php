	...
	(function($){
		$(document).ready(function() {
			...
			convert.modalSteps({
				...
				callbacks: {
					'1': function (){
						var typeFrom  = <?php echo $this->typeId; ?>;
						var typeField = $('#convert_type');
						typeField.find('option[value="' + typeFrom + '"]').attr('disabled', 'disabled');
					...
					'3': function (){
						...
						$.ajax({
							url  : 'index.php?option=com_reditem&task=item.ajaxGetEditFields',
							data : {
								typeTo   : typeTo,
								typeFrom : typeFrom
							},
			...
			convert.on('shown', function() {
				var items = $('input[name="ritem[itemIds][]"]:checked');
