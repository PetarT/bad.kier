...
//require_once (JPATH_ROOT . '/' . 'administrator' . '/' . 'components' . '/' . 'com_sef' . '/' . 'tables' . '/' . 'extension.php');
...
if (SEFTools::JoomFishInstalled()) {
  ...
  JLoader::register('JoomFishManager', JPATH_ROOT. '/' .'administrator'. '/' .'components'. '/' .'com_joomfish'. '/' .'classes'. '/' .'JoomfishManager.class.php');
}
...
class SEFTools
{
  ...
  function getLangCode($langTag = null)
  {
    ...
    if (SEFTools::JoomFishInstalled()) {
      ...
      // Only one language
      $code = null;
      if ($langTag == $defaultTag) {
        $parts = explode('-', $defaultTag);
        $code = $parts[0];
      }
    }
  ...
  function &getExtParams($option)
  {
    ...
    */
    $plugin = JPluginHelper::getPlugin( 'sh404sefextjoomsef', $option);
    $rawParams = empty( $plugin) ? '' : $plugin->params;
  ...
  function &getExtsDefaultParamsXML()
  {
    ...
    if (JFile::exists($xmlpath)) {
      ...
      if ($parser->loadFile($xmlpath)) {
        //$xml = & $parser->document;
        $doc =& $parser->document;
        for( $i = 0, $n = count($doc->params); $i < $n; $i++ ) {
          $xml[] =& $doc->params[$i];
        }
      }
    }
  ...
  function GetSEFGlobalMeta() {
    return '194486783fc011cc1b5d4536468376ae'; // sef.global.meta
  }
  ...
  function getSEOStatus()
  {
    ...
    if( !isset($status) ) {
      ...
      $status['sef'] = (bool)$config->get('config.sef');
      $status['mod_rewrite'] = (bool)$config->get('config.sef_rewrite');
  ...
  function getNonSefVars(&$uri, $nonSefVars, $ignoreVars)
  {
    ...
    if (!empty($gNonSef)) {
    if (!empty($ignoreVars)) {
    $ignoreVars = array_merge($gNonSef, $ignoreVars);
    } else {
    $ignoreVars = $gNonSef;
    }
    }
