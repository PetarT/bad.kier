...
class ReditemControllerSearch extends JControllerLegacy
{
	...
	public function ajaxFilter()
	{
		...
		$items           = $data->items;
		$itemsPagination = $data->items_pagination;
		$type            = $data->type;
		...
		if ((strpos($mainContent, '{items_loop_start}') !== false) && (strpos($mainContent, '{items_loop_end}') !== false))
		{
			...
			$useGmapField        = (boolean) $typeParams->get('item_gmap_field', false);

			if ($useGmapField && !empty($filterDistanceFrom) && !empty($filterDistanceValue))
			{
