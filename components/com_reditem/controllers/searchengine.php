...
class ReditemControllerSearchEngine extends JControllerLegacy
{
	 ...
	 * @return  void
	 */
	public function ajaxSwitchSendMail()
	{
		$app    = JFactory::getApplication();
		$user   = ReditemHelperSystem::getUser();
		$id     = $app->input->getInt('id', 0);
		...
		if (!$model->changeSendMail($id, $status, $user->id))
		{
			$return['msg'] = JText::_('COM_REDITEM_SEARCH_ENGINE_ERROR_SWITCH_SENDMAIL_OPTION');
		...
		$return['status'] = 1;
		$return['msg'] = JText::sprintf('COM_REDITEM_SEARCH_ENGINE_SUCCESS_SWITCH_SENDMAIL_OPTION', $id);
