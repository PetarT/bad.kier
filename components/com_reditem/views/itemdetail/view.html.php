	...
	public function display($tpl = null)
	{
		...
		if ($this->item)
		{
			...
			if (isset($categories[$this->item->id]))
			{
				...
				if (JComponentHelper::isEnabled('com_reditemcategoryfields'))
				{
					...
					{
						$catFields = ReditemCategoryFieldsHelper::getFieldsData($category->id);

						$category->fields = empty($catFields) ? array() : $catFields;
					}
				}
			}
		...
		if (JPluginHelper::isEnabled('system', 'twig'))
		{
			...
			if (is_array($this->item->customfield_values))
			{
