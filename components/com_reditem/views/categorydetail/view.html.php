	...
	public function display($tpl = null)
	{
		...
		if ($this->data)
		{
			...
			if (!empty($this->data->items))
			{
				...
				{
					foreach ($item->customfield_values as &$value)
					{
						if (ReditemHelperCustomfield::isJsonValue($value))
						{
		...
		if (JPluginHelper::isEnabled('system', 'twig'))
		{
			$loader = new Twig_Loader_Array(
				array (
					'categorydetail-' . $this->data->id . '.html' => $content
				)
			);
			...
			$this->content = $twig->render(
				...
				array (
					...
					'fields' => isset($this->data->fields) ? $this->data->fields : null,
					'page'  => $_SERVER
				)
			);
		}
