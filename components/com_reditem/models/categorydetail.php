...
class ReditemModelCategoryDetail extends RModel
{
	...
	public function getData()
	{
		...
		$category->useItemGmapField = (boolean) $typeParams->get('item_gmap_field');
		 ...
		 */
		$filterFieldValues = $input->get_Array('filter_customfield', null);
		...
		if (!empty($filterFieldValues))
		{
			...
			foreach ($filterFieldValues as $field => $value)
			{
				if (empty($value))
				{
					unset($filterFieldValues[$field]);
					continue;
				}
				...
				if (is_array($value))
				{
					...
					$filterFieldData[$field] = $tmpValues;
				}
				else
				{
					$filterFieldData[$field] = base64_decode($value);
				}
			}
		...
		if (!empty($items))
		{
			...
			foreach ($items as $item)
			{
				...
				if ($category->useItemGmapField == true)
				{
