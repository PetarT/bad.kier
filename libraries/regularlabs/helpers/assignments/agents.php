...
{
	var $phoneDevices     = null;
	var $tabletDevices    = null;
	var $operatingSystems = null;
	var $browsers         = null;
	...
	{
		return $this->isMobile(array('tablets'));
	}
	 ...
	 * isMobile
	 */
	public function isMobile($rule_sets = array('phones', 'os', 'browers'))
	{
		$rules = array();

		if (in_array('phones', $rule_sets))
		{
			$rules = array_merge(
				$rules,
		...
		if (in_array('tablets', $rule_sets))
		{
			$rules = array_merge(
				$rules,
				$this->getTabletsDevices()
			);
		}

		if (in_array('os', $rule_sets))
		{
			$rules = array_merge(
		...
		if (in_array('browsers', $rule_sets))
		{
			$rules = array_merge(
				$rules,
				$this->getBrowsers()
	...
	{
		if (!is_null($this->phoneDevices))
		{
			return $this->phoneDevices;
		}

		$this->phoneDevices = array(
	 ...
	 */
	private function getTabletsDevices()
	{
		if (!is_null($this->tabletDevices))
		{
	...
	{
		if (!is_null($this->operatingSystems))
		{
	...
	/**
	 * ist of mobile User Agents
	 */
	private function getBrowsers()
	{
		if (!is_null($this->browsers))
		{
