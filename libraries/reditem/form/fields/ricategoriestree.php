...
class JFormFieldRICategoriesTree extends JFormFieldList
{
	...
	public function getInput()
	{
		...
		$categoryId = JFactory::getApplication()->getUserState('com_reditem.global.cid', array());

		// Get the categories list
		$query = $db->getQuery(true)
			->select('c.*')
			->from($db->qn('#__reditem_categories', 'c'))
			->where($db->qn('c.type_id') . ' = ' . $db->q($typeId))
		...
		if (!empty($categoryId))
		{
			$query->where($db->qn('c.id') . ' NOT IN (' . implode(',', $categoryId) . ')');
		}
