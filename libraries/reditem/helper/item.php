...
class ReditemHelperItem
{
	 ...
	 * @param   int     $basicMode   Only replace general tags
	 *
	...
	public static function replaceTag(&$content, $item, $categoryId = 0, $itemId = 0, $basicMode = false)
	{
		...
		// Item's comments tool
		ReditemHelperComments::replaceTag($content, $item);

		// Item rating tool
		ReditemHelperRating::replaceTag($content, $item);

		// Item report tool
		ReditemHelperReport::replaceTag($content, $item);

		// Item watch tool
		ReditemHelperWatch::replaceTag($content, $item);
	...
	public static function replaceCustomfieldsTag(&$content, $item, $customFieldTags = null)
	{
		...
		self::replaceIfCustomFieldTag($content, $item);
	 ...
	 *
	 * @param   int      $itemIds  List of item id
		...
		if (!empty($itemIds))
		{
			...
			foreach ($refs as $ref)
			{
				...
				else
				{
					...
					if ($category->id)
					{
						$dispatcher->trigger('onAfterLoadCategory', array($category));
	 ...
	 *
	 * @param   object  $itemIds  List of item ids
	 *
	 * @return  mixed
	 */
	...
	public static function replaceRelatedItems(&$content, $item)
	{
		...
		$relatedItems = $itemParams->get('related_items', array());
		...
		if ((strpos($subContent, '{related_items_loop_start}') == true) && (strpos($subContent, '{related_items_loop_end') == true))
		{
			...
			foreach ($relatedItems as $relatedItemId)
			{
				...
				if ($tempItem->published == 1 && $tempItem->blocked == 0)
				{
	...
	public static function replaceGeneralTag(&$content, $item, $catid, $itemId)
	{
		...
		if ($itemId)
		{
			$item->itemLink = htmlspecialchars_decode(
					JRoute::_('index.php?option=com_reditem&view=itemdetail&id=' . $item->id . '&cid=' . $categoryId . '&Itemid=' . $itemId)
			);
		}
		else
		{
			$item->itemLink = htmlspecialchars_decode(JRoute::_(ReditemHelperRouter::getItemRoute($item->id)));
		}
	...
	/**
	 * Method for provide {if_cf} and {if_cf_end} tag
	 *
	 * @param   string  &$content  HTML content of template
	 * @param   object  $item      Item object
	 *
	 * @return boolean  True on success. False otherwise.
	 */
	public static function replaceIfCustomFieldTag(&$content, $item)
	{
		if (empty($content))
		{
			return false;
		...
		if (preg_match_all('/{if_cf[^}]*}/i', $content, $matches) <= 0)
		{
			return false;
		}
		...
		foreach ($matches as $match)
		{
			...
			$customfieldTag = isset($tagParams[1]) ? (string) $tagParams[1] : '';
			$customfieldTag = str_replace('}', '', $customfieldTag);

			if (empty($customfieldTag))
			{
				continue;
			}

			if (!isset($item->customfield_values) || empty($item->customfield_values))
			{
				$cfValues = self::getCustomFieldValues($item->id);
				$item->customfield_values = $cfValues[$item->type_id][$item->id];
			}

			$tempContent = explode($match, $content);
			$preContent  = array_shift($tempContent);
			$tempContent = implode('', $tempContent);
			$endTagPos   = strpos($tempContent, '{if_cf_end}');
			$subTemplate = substr($tempContent, 0, $endTagPos);
			$postContent = substr($tempContent, $endTagPos + strlen('{if_cf_end}'));

			/**
			 * This check in 3 case:
			 *
			 * * This customfield is not available for this item
			 * * This customfield is available but it has empty value
			 * * This customfield is available, value is JSON format and empty
			 */
			if (!isset($item->customfield_values[$customfieldTag])
				|| empty($item->customfield_values[$customfieldTag])
				|| (ReditemHelperCustomfield::isJsonValue($item->customfield_values[$customfieldTag]) !== false
				&& empty(array_filter(json_decode($item->customfield_values[$customfieldTag])))))
			{
