...
class ReditemHelperTags
{
	...
	public static function tagReplaceFilter(&$content, $mainCategory, $javascriptCallback = 'reditemFilterAjax')
	{
		...
		// Prepare Type object
		if (!isset($mainCategory->type))
		{
			$typeModel = RModel::getAdminInstance('Type', array('ignore_request' => true), 'com_reditem');
			$mainCategory->type = $typeModel->getItem($mainCategory->type_id);
		}
		...
		// Replace {filter_rating} tag
		self::replaceFilterRating($content, $mainCategory, $javascriptCallback);

		// Replace {filter_distance} tag
		self::replaceFilterDistance($content, $mainCategory, $javascriptCallback);

		// Replace {save_filter} tag
		self::replaceSaveFilter($content, $mainCategory->type);

		return true;
	}

	/**
	 * Replace comment tag on content of template
	 *
	 * @param   string  &$content  Content template
	 * @param   object  $comment   Item data object
	 *
	 * @return  boolean            True on success. False otherwise.
	 */
	public static function tagReplaceComment(&$content, $comment)
	{
		...
		$content = str_replace('{comment_id}', $comment->id, $content);
		$content = str_replace('{comment_comment}', $comment->comment, $content);
		$content = str_replace('{comment_createddate}', $comment->created, $content);
		...
		$content = str_replace('{comment_user_username}', $user->username, $content);
		$content = str_replace('{comment_user_name}', $user->name, $content);
		$content = str_replace('{comment_user_email}', $user->email, $content);
	...
	public static function replaceFilterCategory(&$content, $mainCategory, $javascriptCallback = 'reditemFilterAjax')
	{
		...
		if (preg_match_all('/{filter_category[^}]*}/i', $content, $matches) > 0)
		{
			...
			$filters = $app->input->get_Array('filter_category', null);
			$value   = '';
			...
			foreach ($matches as $match)
			{
				...
				if (!isset($mainCategory->type) || empty($mainCategory->type))
				{
					$typeModel = RModel::getAdminInstance('Type', array('ignore_request' => true), 'com_reditem');
					$mainCategory->type = $typeModel->getItem($mainCategory->type_id);
				}

				$html = ReditemHelperLayout::render($mainCategory->type, $layoutFile, $layoutData, array('component' => 'com_reditem'));
	...
	public static function replaceFilterTitle(&$content, $mainCategory, $javascriptCallback = 'reditemFilterAjax')
	{
		...
		if (preg_match_all('/{filter_title[^}]*}/i', $content, $matches) > 0)
		{
			...
			foreach ($matches as $match)
			{
				...
				$html = ReditemHelperLayout::render($mainCategory->type, 'filters.filter_title', $layoutData, array('component' => 'com_reditem'));
				$content = str_replace($match, $html, $content);
			}
	...
	public static function replaceFilterSearchInFrontend(&$content, $mainCategory, $javascriptCallback = 'reditemFilterAjax')
	{
		...
		if (preg_match_all('/{filter_searchinfrontend[^}]*}/i', $content, $matches) > 0)
		{
			...
			foreach ($matches as $match)
			{
				...
				$html = ReditemHelperLayout::render($mainCategory->type, 'filters.filter_searchinfrontend', $layoutData, array('component' => 'com_reditem'));
				$content = str_replace($match, $html, $content);
			}
	...
	public static function replaceFilterCustomfield(&$content, $mainCategory, $javascriptCallback = 'reditemFilterAjax')
	{
		...
		if (preg_match_all('/{filter_customfield[^}]*}/i', $content, $matches) > 0)
		{
			...
			// Get filter value
			$filter = $app->input->get_Array('filter_customfield', null);
			...
			foreach ($matches as $match)
			{
				...
				$tableName = '#__reditem_types_' . $mainCategory->type->table_name;
		...
		if (preg_match_all('/{filter_relatedcategory[^}]*}/i', $content, $matches) > 0)
		{
			...
			foreach ($matches as $match)
			{
				...
				if ($subCategories)
				{
					...
					$filters = JFactory::getApplication()->input->get_Array('filter_category', null);
	...
	public static function replaceFilterRanges(&$content, $mainCategory, $javascriptCallback = 'reditemFilterAjax')
	{
		...
		if (preg_match_all('/{filter_ranges[^}]*}/i', $content, $matches) > 0)
		{
			...
			foreach ($matches as $match)
			{
				...
				$html = ReditemHelperLayout::render($mainCategory->type, 'filters.filter_ranges', $layoutData, $layoutOption);
				$content = str_replace($match, $html, $content);
			}
	...
	/**
	 * Replace {filter_rating} tag on content of template
	 *
	 * @param   string  &$content            Content template
	 * @param   object  $mainCategory        Array data of current category
	 * @param   string  $javascriptCallback  Javascript function callback for filter.
	 ...
	 * @return  boolean                      True on success. False otherwise.
	 */
	public static function replaceFilterRating(&$content, $mainCategory, $javascriptCallback = 'reditemFilterAjax')
	{
		// Check if category object is available
		if (empty($content) || !$mainCategory || (strpos($content, '{filter_rating}') === false))
		{
			return false;
		}
		 ...
		 * {filter_rating}
		 */
		$filterValue = JFactory::getApplication()->input->getRaw('filter_rating', null);
		$typeParams = new JRegistry($mainCategory->type->params);
		$filterConfig = array(
			'stars' => $typeParams->get('itemRatingMaxStars', 5),
			'step'  => $typeParams->get('itemRatingStep', 1)
		);
		...
		$html = ReditemHelperLayout::render($mainCategory->type, 'filters.filter_rating', $layoutData, array('component' => 'com_reditem'));
		$content = str_replace('{filter_rating}', $html, $content);
	...
	public static function replaceFilterDistance(&$content, $category, $javascriptCallback = 'reditemFilterAjax')
	{
		...
		$html = ReditemHelperLayout::render($category->type, 'filters.filter_distance', $layoutData, array('component' => 'com_reditem'));
	...
	public static function replaceSortTool(&$content, $category, $javascriptCallback = 'reditemFilterAjax')
	{
		...
		$itemsSort = $input->getString('items_sort', 'title');
		$itemsDest = $input->getString('items_dest', 'asc');
		...
		$html    = ReditemHelperLayout::render($type, 'items_sort', $layoutData, array('component' => 'com_reditem'));
	...
	/**
	 * Method for generate Save Filter button
	 *
	 ...
	 * @return  boolean            True on success. False otherwise.
	 */
	public static function replaceSaveFilter(&$content, $type)
	{
		...
		{
			$html = ReditemHelperLayout::render($type, 'features.savefilter', array('type' => $type), array('component' => 'com_reditem'));
		}

		$content = str_replace('{save_filter}', $html, $content);
	...
	public static function replaceFilterAlphabet(&$content, $category, $jsCallback = 'reditemFilterAjax')
	{
		...
		if (preg_match_all('/{filter_alphabet[^}]*}/i', $content, $matches) > 0)
		{
			...
			$db = JFactory::getDbo();
			$query = $db->getQuery(true)
				->select('UPPER(LEFT(' . $db->qn('title') . ', 1)) AS ' . $db->qn('alphabet'))
			...
			foreach ($matches as $match)
			{
				...
				$html    = ReditemHelperLayout::render($category->type, 'filters.filter_alphabet', $layoutData, array('component' => 'com_reditem'));
				$content = str_replace($match, $html, $content);
			}
